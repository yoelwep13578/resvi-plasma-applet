...
CONFIG_FILE="$SCRIPT_DIR/../config/disk-config.sh"
SCRIPTS_DIR="$SCRIPT_DIR/../scripts"

# Baca file konfigurasi
source "$CONFIG_FILE"

# Ambil data Disk dari script
disk_data=$("$SCRIPTS_DIR/disk.sh" "$disk_target")
disk_usage_kb=$(echo "$disk_data" | cut -d',' -f1)
disk_total_kb=$(echo "$disk_data" | cut -d',' -f2)
disk_usage_percent=$(echo "$disk_data" | cut -d',' -f3)
...
if [[ $loadinfo_type == "percentage" ]]; then
    loadinfo_value="$disk_usage_percent"
    loadinfo_unit="%"
else
    converted_data=$(convert_to_unit "$disk_usage_kb" "$loadinfo_data_size" "$loadinfo_data_unit")
...
if [[ $maxcap_type == "follow-system" ]]; then
    converted_maxcap=$(convert_to_unit "$disk_total_kb" "$loadinfo_data_size" "$loadinfo_data_unit")
...
for part in "${display_order[@]}"; do
    case $part in
        ...
        "load")
            ...
            ;;
        "bar")
            draw_bar "$bar_fill_character" "$bar_empty_character" "$bar_length_num" "$disk_usage_percent" "$bar_left_sign" "$bar_right_sign"
            ;;
        "max")
